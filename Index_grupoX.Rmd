...
6. Cantidad de patentes registradas (patentes)
...
Como se observa en el gráfico, existe una ligera tendencia positiva entre la variable independiente "cantidad de patentes" y la variable dependiente "exportaciones chinas". Parece implicar que, a mayor cantidad de patentes registrdas, los países reciben mayor número de exportaciones chinas, como es el caso de Estados Unidos, Japón, Vietanm, entre otros.Si observamos su gasto en miles de $ junto a su cantidad de patentes observamos tal correlación.
...
```{r}
library(BBmisc)
boxplot(normalize(base[,c(3:8)],method='standardize'))
base[,c(2:13)]=normalize(base[,c(3:8)],method='standardize')
```
...
poorPAM=silPAM[silPAM$sil_width<0,'asociados']
poorPAM
```
...
```{r}
fviz_dend(res.agnes, cex = 0.4, horiz = F,main = "DENDOGRAMA DE LOS PAÍSES QUE RECIBEN EXPORTACIONES CHINAS")
```
...
poorAGNES=silAGNES[silAGNES$sil_width<0,'asociados']#%>%sort()
poorAGNES
```
...
MAPA: PAÍSES SEGÚN CONGLOMERADOS
